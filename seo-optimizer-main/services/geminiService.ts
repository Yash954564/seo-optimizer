...
const generatePrompt = (mainUrl: string): string => {
    const prompt = `
  Analyze the SEO and performance of the website: ${mainUrl}.
  
  IMPORTANT: Use your search capabilities to gather real information about the website at ${mainUrl}. Your entire analysis must be based on publicly available information, not simulation.
  
  Your role is to be an initial SEO auditor. Your goal is to provide a high-level analysis and then guide the user toward free, specialized tools for a deeper, more accurate dive into specific data points.
  
  Provide a comprehensive SEO report. Base your analysis on established SEO best practices. The output must be a JSON object that strictly follows the provided schema.
  
  Your analysis must include:
  1. **Scores (0-100)**: On-Page SEO, Content Quality, Backlinks Profile, and a Readability score. 
     - For each, provide a score and a breakdown of positive factors. 
     - For the Backlinks score, also include a 'topBacklinks' array with 3-5 examples of high-quality backlinks found through search. 
     - In the breakdown, recommend using a tool like "Ahrefs' Free Backlink Checker" for a more complete profile.
  
  2. **Technical SEO Audit**:
     - **Robots.txt & Sitemap.xml Analysis**: Check for presence and validity. Provide recommendations. For deeper analysis, recommend the user inspect their "Google Search Console" account.
     - **Broken Links (404s)**: Based on your search, identify up to 3 broken internal links. For a full site crawl, recommend a tool like "Screaming Frog SEO Spider" (free for up to 500 URLs).
     - **Google Analytics Check**: Check for the presence of a Google Analytics tracking code. Recommend setting it up if absent, or exploring user behavior reports in "Google Analytics (GA4)" if present.
     - **Google Search Console Check**: Infer if GSC is likely set up. Recommend verification and sitemap submission if not, and highlight the value of its performance reports if it is.
     - **Page Speed**: Briefly assess page speed based on observable factors. For detailed metrics and Core Web Vitals, strongly recommend using "Google PageSpeed Insights".
  
  3. **Keyword Analysis**: Identify 5-7 primary keywords. Provide an *estimated* rank on www.google.com for the target location, search volume (Low, Medium, High), and competition. Clarify that these are estimates and for precise data, consult "Google Search Console" (for their own site's performance) and "Google Keyword Planner" (for research).
  
  4. **Competitor Deep Dive**: For each unique competitor, provide a summary of their strategy. Acknowledge that this is based on public information and recommend tools like Semrush (with a free tier) for deeper competitive intelligence.
  
  5. **Actionable Recommendations**: A list of 5-7 high-impact steps. Wherever possible, associate a recommendation with a free tool for verification or implementation (e.g., "Improve mobile usability - check the 'Mobile Usability' report in Google Search Console.").
  
  6. **Content Briefs**: 2-3 content briefs.
  
  7. **Keyword Gaps**: 4-6 keywords that competitors seem to rank for.
  
  8. **Keyword Suggestions**: 3-4 strategic keyword suggestions.
  
  9. **Advanced Suggestions**: 4 strategic suggestions.
  
  Acknowledge the limitations of this analysis, as it is based on publicly available data. Generate plausible but realistic data for all fields where real data is not available. The analysis should feel like it was generated by a professional SEO tool. Do not include any explanatory text outside of the JSON object.
  `;
