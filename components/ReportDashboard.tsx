...
export const ReportDashboard: React.FC<ReportDashboardProps> = ({ report, onDownloadPdf, isPdfMode, isReportLocked, reportUrlId, showNotification }) => {
  // Defensive check: Ensure the report object has the essential 'scores' property before rendering.
  // This prevents the app from crashing if an incomplete report object is passed,
  // which can happen when loading a shared report that might be malformed in the database.
  if (!report.scores) {
    return (
        <div className="mt-8 text-center bg-yellow-100 border border-yellow-400 text-yellow-700 px-4 py-3 rounded-lg animate-fade-in" role="alert">
          <p className="font-bold">Report Data Missing</p>
          <p className="text-sm">The data for this report seems to be incomplete or corrupted. Unable to display details.</p>
        </div>
    );
  }
  ...
  const handleShare = () => {
    if (reportUrlId) {
      const shareUrl = `${window.location.origin}${window.location.pathname}?urlid=${reportUrlId}`;
  ...
  return (
    <div className="mt-8 space-y-12">
      <div style={childAnimation(0)} className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 p-4 bg-white rounded-lg shadow-lg border border-slate-200">
        ...
        {!isLocked && reportUrlId && (
            <button
                onClick={handleShare}
                className="flex items-center gap-2 bg-slate-100 hover:bg-slate-200 text-text-secondary font-bold py-2 px-4 rounded-lg transition-colors border border-slate-300"
            >
